...
export interface Character {  // Declaración local
  id: number;
  name: string;
  image: string;
  isFavorite: boolean;
...
const CharacterList: React.FC<CharacterListProps> = ({ characters, onToggleFavorite }) => (
  <div className="grid grid-cols-3 gap-4 p-6">
    {characters.map((character) => (
      <div key={character.id} className="bg-gray-200 p-4 rounded-lg shadow-md hover:shadow-lg">
        <Link href={`/character/${character.id}`}>  
          <img src={character.image} alt={character.name} className="w-full h-40 object-cover" />
